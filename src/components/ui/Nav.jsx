...
const Nav = () => {
  ...
  return (
    ...
    <div className="bg-gradient-to-r from-darkGreen to-lightGreen text-textWhite uppercase sticky top-0 w-full z-20 flex items-center justify-between py-10 ">
      <div className="large-screen-padding flex justify-between items-center w-screen mob-def-pad">
        <Link href={"/"}>
          <Image
            src="/bidaal.svg"
            alt="logo"
            width={150}
            height={150}
            className="text-white shrink-0 min-w-[150px] min-h-[15px] 2xl:min-w-[250px]"
          />
        </Link>
        <ul className="flex items-center justify-evenly  max-md:flex-col max-md:gap-y-2 gap-x-14 bg-navBtnClr py-2 px-5 2xl:py-4 2xl:px-5 rounded-lg font-semibold max-lg:hidden 2xl:text-3xl">
          {routeLinks.map((link) => (
            <li key={link.label}>
        ...
        <div className=" max-lg:hidden">
          <LinkButton href="/contact" className={"2xl:text-2xl 2xl:px-14 2xl:py-4"}>contact us</LinkButton>
        </div>
